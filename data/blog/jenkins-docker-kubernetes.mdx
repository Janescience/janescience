...
ใน minikube จะไม่ Support External-IP เราต้อง Forward port อีกที และการ Forward port ไม่รองรับ Load balance
...
ถ้าอยากทดสอบ Load balance ต้องเปิด Tunnel
...
❗  The service/ingress discovery-svc requires privileged ports to be exposed: [80]
🔑  sudo permission will be asked for it.
🏃  Starting tunnel for service discovery-svc.
❗  The service/ingress search-svc requires privileged ports to be exposed: [80]
🔑  sudo permission will be asked for it.
🏃  Starting tunnel for service search-svc.
...
ลอง Get Service อีกรอบ จะเห็นว่าได้ 127.0.0.1 ขึ้นมาใน EXTERNAL-IP
...
ถ้าเราทดสอบ Load Test ด้วยการเปิด Tunnel ถึงจะมีการทำ Load Balance ให้
