...
- **Request** ทุก Request ต้องมี Request Headers `X-XSRF-TOKEN`
  - URLs ที่ไม่ต้อง Authen (`.requestMatchers(...).permitAll()`) อย่างเช่น `/signup`, `signin`
    สามารถใช้ Token เดิมได้ตลอด ไม่ต้อง Random ใหม่ แต่ต้องมี `X-XSRF-TOKEN` ในทุก Request > แล้วการ Reuqest ครั้งแรก จะเอา Token มาจากไหน ? นั่นแหล่ะ แสดงว่าเมื่อเข้า Website ในครั้งแรกจะต้องมีการร้องขอสร้าง CSRF Token ไว้รอ
